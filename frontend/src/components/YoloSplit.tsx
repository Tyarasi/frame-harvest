...
export function YoloSplit({ projectId }: Props) {
  ...
  // nama kelas TIDAK di-hardcode "person" lagi — disimpan per-project
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((cfg) => setClassName(cfg.yolo_class_name))
      .catch(() => {})
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api.saveDatasetPrepConfig(projectId, { yolo_class_name: className }).catch(() => {})
    }, 500)
    return () => clearTimeout(timer)
  }, [projectId, configLoaded, className])
  ...
  return (
    <div>
      ...
      <div className="mb-4 max-w-xs">
        <label className="mb-1 block text-xs font-medium text-slate-400">Nama Kelas</label>
        <input
          type="text"
          value={className}
          onChange={(e) => setClassName(e.target.value)}
          placeholder="mis. lanyard, helm, apd"
        ...
        <p className="mt-1 text-[11px] text-slate-600">
          Nama tampilan untuk object yang mau dideteksi — ditulis ke <code>data.yaml</code>, bukan
          dikunci ke "person" lagi.
        </p>
      </div>
