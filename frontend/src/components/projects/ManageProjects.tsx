...
import type { DatasetTarget, Project } from '../../api'
import { DATASET_TARGET_OPTIONS } from '../../recipes'
...
interface Props {
  projects: Project[]
  selectedId: string
  onAdd: (name: string, datasetTarget: DatasetTarget) => Promise<void>
...
export function ManageProjects({ projects, selectedId, onAdd, onRename, onDelete, onDone }: Props) {
  ...
  const [datasetTarget, setDatasetTarget] = useState(DATASET_TARGET_OPTIONS[0].value)
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError(null)
    try {
      // dikunci SETELAH dibuat (tidak ada endpoint untuk mengubahnya) — tapi
      // dipilih bebas SAAT membuat, sejak resep kedua (YOLO) ada
      await onAdd(name, datasetTarget)
      setName('')
    } catch (err) {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-slate-800 bg-slate-900 p-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-slate-400">Tujuan Dataset</label>
          {/* menentukan langkah persiapan dataset apa saja yang berlaku
              untuk project ini (lihat recipes.ts) — TIDAK BISA diganti lagi
              setelah project dibuat, jadi pilih yang benar dari awal. */}
          <select
            value={datasetTarget}
            onChange={(e) => setDatasetTarget(e.target.value as typeof datasetTarget)}
          ...
          >
            {DATASET_TARGET_OPTIONS.map((opt) => (
          ...
          <p className="mt-1 text-[11px] text-slate-600">
            Tidak bisa diubah setelah project dibuat — menentukan langkah persiapan dataset yang berlaku.
          </p>
        </div>
